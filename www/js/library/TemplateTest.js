...
TemplateExtractor = {
  ...
  parseComment : function (o) {
    var string = o.comment.nodeValue;
  ...
  show : function (t) {
    var a = [];
    for (var i=0; i<t.commentParams.length; i++) {
      var item = t.commentParams[i];
      var out = {start: {}, end: {}};
      if (item.type == start)
        out.start.
    }
    var o = {
      name: t.name,
      params: a,
      orderedParams: t.orderedParams,
      tagName: t.elem.tagName
    };
  },
  
  //-------------------------------------------------------------
  extractTemplates : function (output, node, indent, strip) {
          ...
          if ((v = node.attributes.item(i).nodeValue) != null) { 
            if (node.attributes.item(i).nodeName == "template") {
              ...
              if (a.length == 2) {
                item.name = a[0];
                var a2 = a[1].split(")");
                item.orderedParams = a2[0].split(',');
                output.templates.push(item);
              ...
              //attributes.push(this.findAttributeTags(node.attributes.item(i)));
              //{elem: node, attr: node.attributes.item(i), indent: indent});
            }
          }
        }
        for (var i=0; i<node.childNodes.length; i++)
          this.extractTemplates(output, node.childNodes.item(i), indent+1, strip);
        break;
      }
      
      case 8: { // COMMENT_NODE
        var item = {comment: node, indent: indent};
        if (this.parseComment(item) == true) {
          var tmpNode = node;
          while (tmpNode.parentNode != null) {
            ...
            if (tmpNode.templateDef != null) {
              ...
              var cp;
              if ((cp = tmpNode.templateDef.commentParams[item.name]) === undefined)
                cp = tmpNode.templateDef.commentParams[item.name]
              tmpNode.templateDef.commentParams[item.name] = item;
            }
          }
        }
        break;
